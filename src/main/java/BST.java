...
public boolean isAVL() {
    return isBST(this.root) && isAVL(this.root);
}
...
public void recursiveAdd(int element) {
    this.root = recursiveAdd(this.root, element);
    this.size++;
}
 ...
 * @param element elemento a ser adicionado.
 */
private Node recursiveAdd(Node node, int element) {
    
    if (node == null) {
    return new Node(element); // nó recém-criado
    }
...
private Node rotateLeft(Node x) {
    Node y = x.right;      // filho direito vira a nova raiz
    Node T2 = y.left;      // subárvore esquerda de y vai virar filho direito de x

    // Executa rotação
    y.left = x;
    x.right = T2;

    // Atualiza pais (se você usa parent)
...
private Node rotateRight(Node y) {
    Node x = y.left;       // filho esquerdo vira a nova raiz
    Node T2 = x.right;     // subárvore direita de x vai virar filho esquerdo de y
